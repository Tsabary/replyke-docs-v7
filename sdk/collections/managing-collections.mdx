...
`isEntityInCollection` queries whether a specific entity is saved anywhere in the user's collections. Optionally pass a `collectionId` to check a specific folder.

```tsx
const result = await isEntityInCollection(entity.id);
// result.saved — true if saved in any collection
// result.collections — list of { id, name } for each collection containing it

const specificResult = await isEntityInCollection(entity.id, collection.id);
