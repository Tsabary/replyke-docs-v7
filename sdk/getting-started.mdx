...
Use the `useAuth` hook to access the current user's authentication state anywhere inside `ReplykeProvider`:

```tsx
import { useAuth } from "@replyke/react-js";

function Header() {
  const { initialized, currentUser, signOut } = useAuth();

  if (!initialized) return <p>Loading...</p>;
  if (!currentUser) return <p>Not signed in.</p>;
...
If your application already has its own Redux store and you want to merge them, see [Redux Integration](/sdk/redux-integration) for how to use `ReplykeIntegrationProvider` instead.
